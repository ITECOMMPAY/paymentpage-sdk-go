...
func TestCallbackFormats(t *testing.T) {
	cases := []string{
		`{
			"general": {
		...
		`{
			"general": {
				"signature": "d7MQfAOQwkqhj3I3JvFkiQBcdoXvdudUkZelkkxyURKFB5QF+7n/PDiKYcxVOElpOS5apxIAKE7ViqFYUmhqfQ==",
				"project_id": "[2] projectId"
			},
			"request": {
				"id": "[3] requestId",
	...
	signatureHandler := NewSignatureHandler("123")

	for _, callbackData := range cases {
		...
		t.Log("Checks, that basic callback usage without errors")
		callback.GetPayment()
